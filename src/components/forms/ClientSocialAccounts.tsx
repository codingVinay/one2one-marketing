...
const ClientSocialAccounts = ({ clientId, onAccountsChange }: ClientSocialAccountsProps) => {
  ...
  const fetchSocialAccounts = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, provider, account_name, username, is_active, expires_at, last_synced_at, sync_status, sync_error')
  ...
  const accountFor = (provider: string) =>
    accounts.find((a) => a.provider === provider && a.is_active);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {ORDER.map((provider) => {
          ...
          return (
            <Card key={provider} className={unavailable && !account ? 'opacity-70' : undefined}>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Icon className="h-5 w-5" />
                  {platformNames[provider]}
                  {account && (
                    <Badge variant={expired ? 'destructive' : 'default'} className="ml-auto">
                ...
                {account ? (
                  <>
                    ...
                    {expired && (
                      <p className="text-sm text-destructive">
                        Token expired. Reconnect to resume data collection.
                      </p>
                    )}
                    <div className="flex gap-2 pt-1">
                      <Button
                        ...
                        className="flex-1"
                      >
                        {isBusy ? 'Reconnecting...' : (<><LinkIcon className="h-4 w-4 mr-1" />Reconnect</>)}
                      </Button>
                      <Button
                        size="sm"
                        variant="destructive"
                  ...
                  <>
                    ...
                    <Button
                      ...
                      className="w-full"
                    >
                      {isBusy ? 'Connecting...' : (<><LinkIcon className="h-4 w-4 mr-2" />Connect Account</>)}
                    </Button>
                  </>
                )}
              </CardContent>
